...
class AudioService {
  ...
  async generateAudio(text, language = "en", voice = "default") {
    try {
      ...
      try {
        ...
        console.log("TikTok TTS failed, trying system TTS:", tikTokError.message);
        try {
          // Try system TTS
          audioData = await this.generateWithSystemTTS(text, language);
          console.log("System TTS generation successful");
        } catch (systemError) {
          console.log("System TTS failed, creating valid MP3:", systemError.message);
  ...
  async generateWithTikTokTTS(text, language) {
    ...
    try {
      ...
      const response = await fetch("https://api16-normal-c-useast1a.tiktokv.com/media/api/text/speech/invoke/", {
        method: "POST",
        headers: {
  ...
  createMinimalMP3(duration) {
    ...
    for (let i = 0; i < frames; i++) {
      mp3Header.copy(buffer, i * frameSize, 0, Math.min(mp3Header.length, frameSize));
    }
